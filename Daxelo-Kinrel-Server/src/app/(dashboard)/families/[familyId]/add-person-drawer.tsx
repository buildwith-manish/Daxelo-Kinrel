...
import { Loader2, Check, ChevronsUpDown, AlertTriangle, UserPlus } from 'lucide-react'
...
export function AddPersonDrawer({ familyId, open, onOpenChange, onSuccess }: AddPersonDrawerProps) {
  ...
  // Fetch kinship options for the searchable picker
  useEffect(() => {
    async function fetchKinship() {
      try {
        ...
        // Silently fail — picker will just have fewer options
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast({
          title: 'Error',
          description: data.error || 'Failed to add person',
          variant: 'destructive',
        })
        return
      }
      ...
      toast({
        title: 'Person added',
        description: `${form.name} has been added to the family.`,
      })
  ...
  const formContent = (
    <div className="space-y-5 px-1">
      ...
      <div className="space-y-2">
        <Label htmlFor="add-name" style={{ color: 'var(--kinrel-silver)' }}>
      ...
      {/* Date of Birth */}
      <div className="space-y-2">
        <Label htmlFor="add-dob" style={{ color: 'var(--kinrel-silver)' }}>
          Date of Birth
        </Label>
        <Input
          id="add-dob"
      ...
      {/* City */}
      <div className="space-y-2">
        <Label htmlFor="add-city" style={{ color: 'var(--kinrel-silver)' }}>
          City
        </Label>
        <Input
          id="add-city"
      ...
      {/* Occupation */}
      <div className="space-y-2">
        <Label htmlFor="add-occupation" style={{ color: 'var(--kinrel-silver)' }}>
          Occupation
        </Label>
        <Input
          id="add-occupation"
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <Label htmlFor="add-gotra" style={{ color: 'var(--kinrel-silver)' }}>
            Gotra
          </Label>
          <TooltipProvider>
            <Tooltip>
      ...
      <div className="space-y-2">
        ...
        >
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="alive" id="add-alive" />
            <Label htmlFor="add-alive" style={{ color: 'var(--kinrel-white)' }}>Alive</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="deceased" id="add-deceased" />
            <Label htmlFor="add-deceased" style={{ color: 'var(--kinrel-white)' }}>Deceased</Label>
          </div>
        </RadioGroup>
      </div>
  ...
  // Mobile: bottom sheet (Sheet), Desktop: side dialog (Dialog)
  if (isDesktop) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[480px] max-h-[85vh] overflow-y-auto" style={{ backgroundColor: 'var(--kinrel-card)', borderColor: 'rgba(255,255,255,0.1)' }}>
          <DialogHeader>
            <DialogTitle className="text-kinrel-white">Add Person</DialogTitle>
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <SheetHeader>
          <SheetTitle className="text-kinrel-white">Add Person</SheetTitle>
