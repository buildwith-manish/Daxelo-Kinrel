-- =============================================================================
-- Truth or Dare participant fallback + archive fixes
-- =============================================================================
-- ROOT CAUSE (deeper than 20260915170000): the Truth or Dare lobby creates
-- rooms via its own provider (todProvider.createGame writes truthordare_
-- players only) instead of the shared RoomController — so game_participants
-- is ALWAYS empty for Truth or Dare and the archive processor's participant
-- step returned NULL for every match. Combined with the open-ended party
-- nature fixed in 20260915170000, this made Truth or Dare matches archive-
-- proof: no history, stats, badges, challenges, milestones or Cup points.
--
-- FIX: fn__archive_family_match now derives Truth or Dare participants from
-- truthordare_players when game_participants is empty (host detected via the
-- game row's hostUserId). This covers every creation/join path.
-- =============================================================================
