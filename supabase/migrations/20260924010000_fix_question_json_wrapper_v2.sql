-- 20260924010000_fix_question_json_wrapper_v2.sql
--
-- Fix v2: the to_jsonb(q.*) approach still produces a wrapper column
-- {"to_jsonb": {...}} because SELECT ... INTO stores the column name.
--
-- The real fix: don't use SELECT ... INTO for the question JSON.
-- Instead, build the question JSON inline using jsonb_build_object
-- with explicit column references. This avoids the column-name
-- wrapper entirely.
--
-- Three places need fixing in fn_pb_v1_get_next_question:
--   1. Existing open round path (was line 68)
--   2. Revealed round fallback path (was line 95)
--   3. New round path (was line 194 — uses to_jsonb(v_question) which
--      also wraps since v_question is a record variable)
...
BEGIN
  ...
  IF FOUND THEN
    -- FIX v2: build the question JSON inline with jsonb_build_object
    -- instead of SELECT to_jsonb(q.*) INTO, which produces a
    -- {"to_jsonb": {...}} wrapper.
    SELECT jsonb_build_object(
      ...
      'category', q.category,
      'fun_fact_text', q.fun_fact_text,
      'min_bound', q.min_bound,
      'max_bound', q.max_bound,
      'is_active', q.is_active,
      'difficulty', q.difficulty
    ) INTO v_question_json
    FROM "pb_v1_questions" q WHERE q.id = v_existing_round.question_id;
  ...
  IF now() >= v_reveal_at THEN
    ...
    IF FOUND THEN
      ...
      FROM "pb_v1_questions" q WHERE q.id = v_revealed_round.question_id;
  ...
  -- FIX v2: build the question JSON inline with jsonb_build_object
  -- for the new-round path too (was to_jsonb(v_question) which wraps).
  RETURN jsonb_build_object(
    'ok', true,
    ...
    'question', jsonb_build_object(
      ...
      'unit_label', v_question.unit_label,
      'category', v_question.category,
      'fun_fact_text', v_question.fun_fact_text,
      'min_bound', v_question.min_bound,
      'max_bound', v_question.max_bound,
