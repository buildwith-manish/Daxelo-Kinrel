// scripts/test_kinrel_event_listener.ts
//
// Kinrel Phase 6 — Validation Script for KinrelEventListener
//
...
//   bun ../scripts/test_kinrel_event_listener.ts
//
// Tests:
...
import { KinrelModule } from '../server/src/kinrel-intelligence/kinrel.module';
import { KinrelEventListener } from '../server/src/kinrel-intelligence/kinrel-event.listener';
import { PrismaService } from '../server/src/prisma/prisma.service';

const TEST_FAMILY_ID = 'cmr1xhyo7bivcw8rzx0hlguyi';

@Module({
  imports: [PrismaModule, EventEmitterModule.forRoot(), KinrelModule],
})
class TestAppModule {}

async function main() {
  console.log('Kinrel Phase 6 — Event Listener Validation\n');
  const app = await NestFactory.createApplicationContext(TestAppModule, {
    logger: ['error', 'warn', 'log'],
  });
  const eventEmitter = app.get(EventEmitter2);
  const listener = app.get(KinrelEventListener);
  const prisma = app.get(PrismaService);

  // Clean up
  await prisma.familyKinrel.deleteMany({ where: { familyId: TEST_FAMILY_ID } });
  await prisma.familyKinrelHistory.deleteMany({ where: { familyId: TEST_FAMILY_ID } });
  await prisma.memberKinrelRole.deleteMany({ where: { familyId: TEST_FAMILY_ID } });
  ...
  await sleep(2000);
  const kinrel1 = await prisma.familyKinrel.findUnique({ where: { familyId: TEST_FAMILY_ID } });
  const hist1 = await prisma.familyKinrelHistory.findFirst({ where: { familyId: TEST_FAMILY_ID }, orderBy: { capturedAt: 'desc' } });
  console.log(`  FamilyKinrel exists: ${kinrel1 !== null}, triggerEventType: ${hist1?.triggerEventType}`);
  console.log(`  Result: ${kinrel1 && hist1?.triggerEventType === 'member_added' ? '✅ PASSED' : '❌ FAILED'}\n`);

  // TEST 2: Debounce
  console.log('TEST 2: 5 rapid events → 1 recompute');
  const before = await prisma.familyKinrelHistory.count({ where: { familyId: TEST_FAMILY_ID } });
  eventEmitter.emit('family.member.added', { familyId: TEST_FAMILY_ID, memberId: realPerson.id });
  await sleep(150);
  ...
  eventEmitter.emit('family.member.added', { familyId: TEST_FAMILY_ID, memberId: realPerson.id });
  await sleep(4000);
  const after = await prisma.familyKinrelHistory.count({ where: { familyId: TEST_FAMILY_ID } });
  ...
  for (const [event, expected] of eventTypes) {
    ...
    await sleep(4500);
    const latest = await prisma.familyKinrelHistory.findFirst({ where: { familyId: TEST_FAMILY_ID }, orderBy: { capturedAt: 'desc' } });
  ...
  await cleanupPrisma.familyKinrel.deleteMany({ where: { familyId: TEST_FAMILY_ID } });
  await cleanupPrisma.familyKinrelHistory.deleteMany({ where: { familyId: TEST_FAMILY_ID } });
  await cleanupPrisma.memberKinrelRole.deleteMany({ where: { familyId: TEST_FAMILY_ID } });
