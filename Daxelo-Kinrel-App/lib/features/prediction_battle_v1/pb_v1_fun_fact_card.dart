...
class _FamilyCoinPoolCardState extends ConsumerState<FamilyCoinPoolCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final progress = (_totalEarned! / _currentGoal!).clamp(0.0, 1.0);
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            KinrelColors.brightGold.withValues(alpha: 0.08),
            KinrelColors.darkCard,
          ],
        ),
        borderRadius: BorderRadius.circular(14),
        border: Border.all(color: KinrelColors.brightGold.withValues(alpha: 0.25), width: 0.8),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              value: progress,
              minHeight: 8,
              backgroundColor: KinrelColors.brightGold.withValues(alpha: 0.15),
              valueColor: const AlwaysStoppedAnimation<Color>(KinrelColors.brightGold),
            ),
          ),
