// lib/features/games/connect4/connect4_provider.dart
//
// Connect 4 — Riverpod state + Supabase Realtime + server-authoritative
// disc drops. Mirrors memorymatch_provider.dart / ashtachamma_provider.dart.
...
class Connect4State {
  ...
  Connect4Player? playerFor(String? userId) {
  ...
  Connect4State copyWith({
    ...
    bool clearOptimistic = false,
      ...
      Connect4State(
        ...
        optimisticColumn:
            clearOptimistic ? null : (optimisticColumn ?? this.optimisticColumn),
      );
}

class Connect4Notifier extends StateNotifier<Connect4State> {
  Connect4Notifier(this._ref, this.familyId)
      : super(const Connect4State());
  ...
  Future<String?> createGame({
    ...
    try {
      ...
      final resp =
          await client.from('connect4_games').insert(body).select().single();
      final game = Connect4Game.fromJson(resp);
      _gameId = game.id;

      await client.from('connect4_players').insert({
        'gameId': game.id,
        'userId': myId,
        'userName': _myName,
      });
      ...
      debugPrint('[Connect4] createGame error: $e');
  ...
  Future<bool> joinGame(String gameId) async {
    ...
    try {
      ...
      if (existing == null) {
        ...
        await client.from('connect4_players').update({
          'leftAt': null,
          'isReady': false,
        }).eq('id', existing['id']);
      }
      ...
      debugPrint('[Connect4] joinGame error: $e');
      state = state.copyWith(isLoading: false, error: '$e');
      return false;
    }
  }
  ...
  Future<String?> startGame() async {
    ...
    if (active < 2) return 'Connect 4 needs 2 players';
    state = state.copyWith(isStarting: true, clearError: true);
    try {
      final client = _client;
      if (client == null) return null;
      final result = await client.rpc('fn_connect4_start', params: {
        'p_game_id': gameId,
      });
      ...
      return switch (map['reason']) {
        'not_enough_players' => 'Connect 4 needs 2 players',
        'not_host' => 'Only the host can start',
        'already_started' => null,
        _ => 'Could not start the game',
      };
    } catch (e) {
      debugPrint('[Connect4] startGame error: $e');
  ...
  Future<void> dropDisc(int column) async {
    ...
    state = state.copyWith(optimisticColumn: column);
    GameMotionTokens.tap();

    try {
      final client = _client;
      if (client == null) return;
      await client.rpc('fn_connect4_drop', params: {
        'p_game_id': gameId,
        'p_column': column,
      });
    } catch (e) {
      debugPrint('[Connect4] drop error: $e');
      state = state.copyWith(clearOptimistic: true);
    }
  }
  ...
  Future<void> leaveGame() async {
    ...
    try {
      if (game != null && game.isWaiting && game.hostUserId == myId) {
        ...
        await _tryRpc('fn_connect4_leave', {'p_game_id': gameId});
      }
    } catch (_) {}
    _cleanup();
  }
  ...
  Future<String?> rematch() async {
    ...
    final roster = List<Connect4Player>.from(state.players);
    ...
    try {
      ...
      if (others.isNotEmpty) {
        await client.from('connect4_players').upsert(
          others
              .map((p) => {
                    'gameId': newGameId,
                    'userId': p.userId,
      ...
      final invites = roster
          ...
          .map((p) => {
                ...
                'currentPlayers': 1,
                'message':
                    '$_myName wants a Connect 4 rematch — ready for round 2?',
                'status': 'pending',
      ...
      debugPrint('[Connect4] rematch error: $e');
    }
    return newGameId;
  }
  ...
  Future<void> _refreshPlayers(String gameId) async {
    ...
    try {
      ...
      final players = resp.map((p) => Connect4Player.fromJson(p)).toList();
      state = state.copyWith(players: players);
    } catch (e) {
      debugPrint('[Connect4] refreshPlayers error: $e');
    }
  }

  Future<bool> _tryRpc(String fn, Map<String, dynamic> params) async {
    if (_tickInFlight && fn == 'fn_connect4_tick') return false;
    final client = _client;
    if (client == null) return false;
    try {
      if (fn == 'fn_connect4_tick') _tickInFlight = true;
      await client.rpc(fn, params: params);
      return true;
    } catch (e) {
      debugPrint('[Connect4] $fn error: $e');
      return false;
    } finally {
      if (fn == 'fn_connect4_tick') _tickInFlight = false;
    }
  }
  ...
  void _subscribeToRealtime(String gameId) {
    ...
    _channel = client
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            _applyGameRow(Connect4Game.fromJson(payload.newRecord));
          },
        )
        .onPostgresChanges(
          event: PostgresChangeEvent.insert,
          schema: 'public',
          table: 'connect4_players',
          filter: PostgresChangeFilter(
            type: PostgresChangeFilterType.eq,
            column: 'gameId',
            value: gameId,
          ...
          callback: (payload) {
            final player = Connect4Player.fromJson(payload.newRecord);
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            final updated = Connect4Player.fromJson(payload.newRecord);
            final next = state.players
...
final connect4Provider = StateNotifierProvider.autoDispose
    .family<Connect4Notifier, Connect4State, String>(
  (ref, familyId) => Connect4Notifier(ref, familyId),
);
