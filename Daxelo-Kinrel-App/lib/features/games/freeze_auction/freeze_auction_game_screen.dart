...
class _FreezeAuctionGameScreenState extends ConsumerState<FreezeAuctionGameScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return DKScaffold(backgroundColor: KinrelColors.darkSurface,
      ...
      body: game.isCompleted ? _ResultsView(game: game, familyId: widget.familyId, players: state.players, isHost: game.hostUserId == myId, onRematch: () => ref.read(freezeAuctionProvider(widget.familyId).notifier).rematch(), onExit: () { if (context.canPop()) context.pop(); else context.go('/family/${widget.familyId}'); })
...
class _ResultsView extends StatelessWidget {
  ...
  final FreezeAuctionGame game; final String familyId; final List<FreezeAuctionPlayerWire> players; final bool isHost;
    ...
    return SingleChildScrollView(padding: const EdgeInsets.all(KinrelSpacing.lg),
      child: Column(children: [
        ...
        Row(children: [
          ...
          if (isHost) ...[
            const SizedBox(width: 10),
            Expanded(child: RematchButton(familyId: familyId, gameType: GameType.freezeAuction, previousGameId: game.id, participantUserIds: players.where((p) => p.isActive).map((p) => p.userId).toList(), maxPlayers: game.maxPlayers, insertInvites: false, onCreateNewGame: onRematch)),
          ],
        ]),
      ]));
  }
