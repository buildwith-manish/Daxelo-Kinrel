// lib/features/games/tictactoe/tictactoe_card.dart
...
class TttCard extends ConsumerStatefulWidget {
  const TttCard({super.key, required this.familyId});
  final String familyId;
  @override
  ConsumerState<TttCard> createState() => _TttCardState();
}

class _TttCardState extends ConsumerState<TttCard> {
  @override
  void initState() { super.initState(); Future.microtask(() => ref.read(gameDownloadStatusProvider('tictactoe').notifier).checkStatus()); }
  @override
  Widget build(BuildContext context) {
    final dlState = ref.watch(gameDownloadStatusProvider('tictactoe'));
    const accent = Color(0xFF8B5CF6); // purple
    return Container(
      ...
      child: Material(color: Colors.transparent, child: InkWell(
        ...
        onTap: () { if (dlState.status != GameDownloadStatus.downloaded) { context.push('/games?familyId=${widget.familyId}'); return; } context.push('/family/${widget.familyId}/tictactoe/lobby'); },
