// lib/features/games/word_forge/word_forge_lobby_screen.dart
//
// Word Forge — Create Room lobby.
...
class WordForgeLobbyScreen extends ConsumerStatefulWidget {
  const WordForgeLobbyScreen({super.key, required this.familyId});
  final String familyId;

  @override
  ConsumerState<WordForgeLobbyScreen> createState() =>
      _WordForgeLobbyScreenState();
}

class _WordForgeLobbyScreenState
    extends ConsumerState<WordForgeLobbyScreen> {
  ...
  Future<void> _createGame() async {
    ...
    await ref.read(wordForgeProvider(widget.familyId).notifier).createGame(
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(wordForgeProvider(widget.familyId), (previous, next) {
      if (next.isInProgress &&
          !(previous?.isInProgress ?? false) &&
          next.game?.id != null &&
          mounted) {
        context.pushReplacement(
          '/family/${widget.familyId}/word-forge/game/${next.game!.id}',
        );
      }
    });
  ...
  void _openInviteSheet(WordForgeState_ state) {
  ...
  Widget _setupView() {
    return LobbySetupScreen(
        ...
        children: [
              ...
              decoration: InputDecoration(
                ...
                focusedBorder: OutlineInputBorder(
                  ...
                  borderSide: const BorderSide(
                      color: Color(0xFF8B5CF6), width: 1.4),
                ),
              ),
            ),
          ),
          ...
          LobbySection(
            ...
            child: LobbyChoiceGrid<int>(
              ...
              options: const [
                ...
                LobbyOption(value: 15, label: '15', caption: 'Marathon'),
              ],
            ),
          ),
          const SizedBox(height: KinrelSpacing.md),
          LobbySection(
            ...
            child: LobbyChoiceGrid<int>(
              ...
              options: const [
                LobbyOption(value: 60, label: '60s', caption: 'Quick'),
      ...
      ctaHint: 'Invite family, then forge definitions!',
      ctaLoading: _creating,
      onCtaPressed: _createGame,
    );
  }

  Widget _lobbyView(WordForgeState_ state, bool isHost) {
    final game = state.game!;
    final myId = ref.read(supabaseProvider)?.auth.currentUser?.id;
    ...
    final config = TemporaryLobbyConfig(
      ...
      status: lobbyStatus,
      subtitle:
          '${game.totalRounds} rounds · ${game.answerSeconds}s · obscure words',
    );
