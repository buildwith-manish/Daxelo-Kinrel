...
class _Tree3DScreenState extends ConsumerState<Tree3DScreen>
  ...
  void _onSearchChanged(String query) {
    // CRITICAL ANR FIX: Separate state update from camera focus animation
    // Previously nested setState calls + _focusOnNode setState caused cascading rebuilds
    setState(() {
      _searchQuery = query;
    ...
    // Auto-focus camera on first match — delayed to avoid nested setState
    if (query.isNotEmpty && _highlightedNodeIds.isNotEmpty) {
  ...
  void _onLayoutAnimationTick() {
    if (!_isAnimatingLayout || !mounted) return;
  ...
  void _switchLayoutMode(Tree3DLayoutMode newMode) {
    // CRITICAL ANR FIX: Add guard to prevent concurrent animations
    if (newMode == _layoutMode && !_isAnimatingLayout) return;
    if (_isAnimatingLayout && newMode != _layoutMode) return; // Prevent interrupting

    // Save current positions as start for animation
    _previousPositions = Map.from(_nodePositions);

    // CRITICAL ANR FIX: Update all state at once, not in multiple setState calls
    setState(() {
      _layoutMode = newMode;
