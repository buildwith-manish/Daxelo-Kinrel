...
class _PathFinderScreenState extends ConsumerState<PathFinderScreen>
  ...
  void _findPath() {
    ...
    // CRITICAL ANR FIX: Consolidated nested setState calls into a single
    // setState block to prevent cascading rebuild loop. Nested whenData
    // callbacks with multiple setState calls on the main thread caused
    // the 60+ recursive loop visible in the ANR stack trace.
    membersAsync.whenData((members) {
      relsAsync.whenData((relationships) {
        ...
        if (mounted) {
          // CRITICAL ANR FIX: Single setState call instead of multiple
          setState(() {
            _isSearching = false;
            if (result != null) {
              _pathResult = result;
          ...
          if (result != null && mounted) {
            _pathRevealController.forward(from: 0).then((_) {
              if (mounted) _resultAppearController.forward(from: 0);
            });
          }
        }
      });
            ...
            children: [
              ...
              _InfoChip(
                ...
                onTap: () {
                  ...
                  if (kEnableAudioPronunciation && result.path.isNotEmpty) {
                    final firstStep = result.path.first;
                    _pronounceTerm(firstStep.type);
                  }
                },
                isButton: true,
              ),
