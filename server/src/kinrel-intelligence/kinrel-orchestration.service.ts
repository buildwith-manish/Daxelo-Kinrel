// server/src/kinrel/kinrel-orchestration.service.ts
//
// Kinrel — Orchestration Service
//
// The main coordinator for Kinrel computation. Wires together:
...
//   3. KinrelParameterGeneratorService → map metrics → symbol params (Phase 4)
//   4. Prisma upsert → store FamilyKinrel row
//   5. Prisma create → push FamilyKinrelHistory snapshot
//   6. RoleGlyphService → compute + upsert MemberKinrelRole rows
//
// Realtime broadcast is automatic: FamilyKinrel + MemberKinrelRole are in the
...
// caller (KinrelEventListener or KinrelController) can decide what to do.
...
@Injectable()
export class KinrelOrchestrationService {
  private readonly logger = new Logger(KinrelOrchestrationService.name);

  constructor(
    private readonly prisma: PrismaService,
    private readonly graphAnalysis: GraphAnalysisService,
    private readonly classifier: ArchetypeClassifierService,
    private readonly paramGenerator: KinrelParameterGeneratorService,
    private readonly roleGlyph: RoleGlyphService,
  ) {}

  /**
   * Compute the full Kinrel for a family and persist all results.
   *
   * Steps:
   *   1. Compute graph metrics
   *   2. Classify archetype
   *   3. Generate symbol parameters
   *   4. Load previous Kinrel (to detect archetype changes)
   *   5. Upsert FamilyKinrel row
   *   6. Create FamilyKinrelHistory snapshot
   *   7. Compute + upsert MemberKinrelRole rows
   *
   * Realtime broadcast happens automatically via the supabase_realtime
   * publication on FamilyKinrel + MemberKinrelRole.
   */
  async computeAndSave(
    ...
    this.logger.log(`Starting Kinrel computation for family ${familyId}`);

    // Step 1: Compute graph metrics
    const metrics = await this.graphAnalysis.computeMetrics(familyId);

    if (metrics.memberCount === 0) {
      this.logger.warn(`Family ${familyId} has 0 members — skipping Kinrel computation`);
      return;
    }
    ...
    // Step 4: Load previous Kinrel to detect archetype changes
    const previousKinrel = await this.prisma.familyKinrel.findUnique({
      where: { familyId },
      select: { archetypeKey: true },
    });

    // archetypeChanged is TRUE only when there was a previous archetype
    // AND it differs from the new one. The first computation (previousKinrel
    // is null) is NOT a change — there was nothing to change from.
    const archetypeChanged =
      previousKinrel !== null &&
      previousKinrel.archetypeKey !== classification.archetypeKey;
    ...
    // constraint on FamilyKinrelHistory.triggerMemberId would fail.
    ...
    // Step 5: Upsert family_kinrel
    await this.prisma.familyKinrel.upsert({
      where: { familyId },
      create: {
        familyId,
    ...
    await this.prisma.familyKinrelHistory.create({
      data: {
        familyId,
        memberCount: metrics.memberCount,
        generationDepth: metrics.generationDepth,
        ...
        // snapshot so the Kinrel Timeline can answer "which language
        ...
        archetypeChanged,
        previousArchetype: archetypeChanged ? (previousKinrel?.archetypeKey ?? null) : null,
        capturedAt: new Date(),
      },
    });
    ...
    this.logger.log(
      `Kinrel computed for family ${familyId} in ${elapsedMs}ms: ` +
