// server/src/kinrel/kinrel-event.listener.ts
//
// Kinrel — Event Listener for Real-Time Recalculation
//
...
// rapid changes (e.g., bulk imports), then triggers a full Kinrel
// recomputation via KinrelOrchestrationService.
//
// Events listened to:
...
//   - This means 100 rapid member adds → 1 Kinrel recompute, not 100.
...
@Injectable()
export class KinrelEventListener implements OnModuleDestroy {
  private readonly logger = new Logger(KinrelEventListener.name);
  ...
  // `computeAndSave` calls would both `prisma.familyKinrel.upsert(...)`
  // + `prisma.familyKinrelHistory.create(...)` → last-writer-wins on
  // the upsert, duplicate history rows, and possibly inconsistent
  // `MemberKinrelRole` rows.
  ...
  constructor(private readonly orchestration: KinrelOrchestrationService) {}
  ...
  // should affect the Kinrel, re-add this handler at that time.
  ...
  private scheduleRecompute(
    ...
    this.logger.debug(
      `Kinrel recompute scheduled for family ${familyId} (trigger: ${trigger.eventType}, memberId: ${trigger.memberId ?? 'none'})`,
    );
    ...
    const timeout = setTimeout(() => {
      ...
      // calls that race on the same `familyKinrel` row.
      if (this.inFlight.has(familyId)) {
        this.logger.debug(
          `Kinrel recompute for family ${familyId} deferred — another compute is in-flight`,
        );
      ...
      this.orchestration
        ...
        .catch((error: unknown) => {
          this.logger.error(
            `Kinrel recompute failed for family ${familyId}: ${
  ...
  onModuleDestroy() {
    ...
    this.inFlight.clear();
    this.logger.log('KinrelEventListener destroyed — cleared all pending recomputes');
  }
