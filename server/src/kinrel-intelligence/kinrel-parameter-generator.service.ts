// server/src/kinrel/kinrel-parameter-generator.service.ts
//
// Kinrel — Parameter Generator (Symbol Math)
//
...
export class KinrelParameterGeneratorService {
  /**
   * Generate the full Kinrel symbol parameters from graph metrics + archetype.
   * Pure function — same inputs always produce the same outputs.
   */
  generate(metrics: GraphMetrics, archetypeKey: ArchetypeKey): KinrelSymbolParameters {
    return {
  ...
  computeColorPalette(
  ...
  ): Pick<KinrelSymbolParameters, 'primaryColorHex' | 'secondaryColorHex' | 'accentColorHex'> {
    const entries = Object.entries(ratios)
      ...
      // recomputes for the same family, making the Kinrel symbol look
