// server/src/kinrel/role-glyph.service.ts
//
// Kinrel — Role Glyph Service
//
// Classifies each family member into one of 6 roles based on their graph
// position, then upserts a MemberKinrelRole row per member.
//
// Roles:
...
// The glyph color is derived from the Kinrel palette (primary/secondary/accent).

import { Injectable, Logger } from '@nestjs/common';
import { PrismaService } from '../prisma/prisma.service';
import { GraphMetrics } from './graph-metrics';
import { KinrelSymbolParameters } from './kinrel-parameter-generator.service';
...
export class RoleGlyphService {
  ...
  /**
   * Compute role classifications for every member and upsert MemberKinrelRole rows.
   *
  ...
  async computeAndSaveRoles(
    ...
    await Promise.all(
      roles.map((role) =>
        this.prisma.memberKinrelRole.upsert({
          where: {
    ...
    // (e.g., if a member was deleted since the last Kinrel computation)
    const roleCounts = await this.prisma.memberKinrelRole.count({
      where: { familyId },
    });
    if (roleCounts > roles.length) {
      await this.prisma.memberKinrelRole.deleteMany({
        where: {
          familyId,
      ...
      this.logger.debug(
        `Deleted ${roleCounts - roles.length} stale MemberKinrelRole rows for family ${familyId}`,
      );
    }
  ...
  /**
   * Map a role to a color from the Kinrel palette.
   ...
   */
  private roleColor(roleKey: RoleKey, params: KinrelSymbolParameters): string {
