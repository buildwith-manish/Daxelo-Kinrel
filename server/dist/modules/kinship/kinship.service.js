...
exports.KinshipService = void 0;
const common_1 = require("@nestjs/common");
const KINSHIP_DATABASE = [
    {
...
let KinshipService = class KinshipService {
    constructor() {
        this.kinshipTerms = new Map();
    ...
    lookup(params) {
        ...
        if (params.search) {
            ...
            results = results.filter((term) => {
                if (term.englishTerm.toLowerCase().includes(query))
                    return true;
                if (term.relationshipKey.toLowerCase().includes(query))
                    return true;
                if (term.aliases?.some((a) => a.toLowerCase().includes(query)))
                    return true;
                for (const lang of Object.values(term.translations)) {
                    if (lang.native.toLowerCase().includes(query) ||
        ...
        if (params.category) {
            results = results.filter((term) => term.relationshipCategory === params.category);
        }
        if (params.gender) {
    ...
    searchByTermAndLang(term, lang, limit) {
    ...
    getByCategory(category) {
        return KINSHIP_DATABASE.filter((term) => term.relationshipCategory === category);
    }
    getRandomTerms(count, category) {
        let pool = category
        ...
        for (const term of KINSHIP_DATABASE) {
            ...
            if (confidence === 0 && term.aliases) {
                for (const alias of term.aliases) {
                    if (alias.toLowerCase().includes(lowerText) ||
...
exports.KinshipService = KinshipService;
exports.KinshipService = KinshipService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [])
], KinshipService);
//# sourceMappingURL=kinship.service.js.map